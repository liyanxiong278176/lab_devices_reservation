...
create(CreateReq req, currentUser):
  ...
  4. 校验 slot_count <= 设备 max_reservation_hours 换算的槽上限
...
- 三角色与职责：
  - **STUDENT**：浏览/预约/取消/查我的预约与通知/报修（报修阶段1可暂以"故障反馈通知"简化，见 §11 待确认）。
  - **LAB_ADMIN**：管理本实验室设备、审批本实验室预约、签到归还、待审批列表。
...
- `Result<T>{code:int, msg:String, data:T}`；成功 code=200（或业务自定义 0/200，统一即可）。
...
7. 单元测试 + 集成测试全部通过；`mvn test` 绿。

---

## 13. 待确认（Open Questions，不阻塞但需在实现前定）

1. **设备报修（故障申报）**：用户端列表里有"设备报修"。阶段1 是否纳入一个最小版（`repair_report` 表 + 提交/查看状态），还是延后到阶段2？**建议**：阶段1 纳入最小版报修（提交 + 状态流转：待处理/处理中/已修复），因为它与设备状态强相关、答辩演示更完整。需用户确认。
2. **dashboard 周趋势**：阶段1 返回近 7 天预约数数组即可，ECharts 渲染阶段3。确认无异议。
3. **批处理/巡检**：爽约自动判定等定时任务阶段1 不做，留扩展点。确认无异议。
