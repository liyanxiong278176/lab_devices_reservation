...
## Task 14: WebSocketConfig + 握手期鉴权（实现修订）

> **实现修订**：原计划用 `AuthChannelInterceptor` 拦 STOMP CONNECT 帧头 `Authorization` 鉴权。实测发现 `convertAndSendToUser` 依赖的 `SimpUserRegistry` 按握手期 Principal 键映射，而浏览器 WS 无法在握手请求加自定义头 → CONNECT 帧设的 Principal 不进 registry（实测 totalUsers=0，推送静默丢弃）。**已改为握手期 query token 鉴权**：`WsAuthHandshakeInterceptor`（从 `?token=` 校验 JWT，放 userId 到 attributes）+ `JwtHandshakeHandler`（覆盖 `determineUser` 设会话 Principal=userId）。删除 `AuthChannelInterceptor`。前端 SockJS URL 带 `?token=`。端到端验证通过（学生建预约实时收到推送）。详见 spec §7.2。下方原 AuthChannelInterceptor 步骤为历史记录，以本说明为准。
